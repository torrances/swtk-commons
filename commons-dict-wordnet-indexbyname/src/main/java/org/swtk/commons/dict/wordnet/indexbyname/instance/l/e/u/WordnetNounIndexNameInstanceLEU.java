package org.swtk.commons.dict.wordnet.indexbyname.instance.l.e.u;  import java.util.ArrayList; import java.util.Collection; import java.util.Map; import java.util.TreeMap;  import org.swtk.common.dict.dto.wordnet.IndexNoun;  import com.trimc.blogger.commons.utils.GsonUtils;  public final class WordnetNounIndexNameInstanceLEU {  	private static Map<String, Collection<IndexNoun>> map = new TreeMap<String, Collection<IndexNoun>>();  	static { 			add("{\"term\":\"leu\", \"synsetCount\":2, \"upperType\":\"NOUN\", \"ids\":[\"13707580\", \"13724336\"]}");
	add("{\"term\":\"leucadendron\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"12240764\"]}");
	add("{\"term\":\"leucadendron argenteum\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"12241013\"]}");
	add("{\"term\":\"leucaemia\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"14266404\"]}");
	add("{\"term\":\"leucaena\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"11782706\"]}");
	add("{\"term\":\"leucaena glauca\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"11782902\"]}");
	add("{\"term\":\"leucaena leucocephala\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"11782902\"]}");
	add("{\"term\":\"leucanthemum\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"12010105\"]}");
	add("{\"term\":\"leucanthemum lacustre\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"12011096\"]}");
	add("{\"term\":\"leucanthemum maximum\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"12010636\"]}");
	add("{\"term\":\"leucanthemum superbum\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"12010782\"]}");
	add("{\"term\":\"leucanthemum vulgare\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"12010338\"]}");
	add("{\"term\":\"leucine\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"14959266\"]}");
	add("{\"term\":\"leuciscus\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"01443590\"]}");
	add("{\"term\":\"leuciscus cephalus\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"01443913\"]}");
	add("{\"term\":\"leuciscus leuciscus\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"01443758\"]}");
	add("{\"term\":\"leucocyte\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"05457535\"]}");
	add("{\"term\":\"leucocytosis\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"14219703\"]}");
	add("{\"term\":\"leucocytozoan\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"01427864\"]}");
	add("{\"term\":\"leucocytozoon\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"01427864\"]}");
	add("{\"term\":\"leucogenes\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"12011273\"]}");
	add("{\"term\":\"leucogenes leontopodium\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"12011389\"]}");
	add("{\"term\":\"leucoma\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"14281065\"]}");
	add("{\"term\":\"leucopenia\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"14219887\"]}");
	add("{\"term\":\"leucorrhea\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"05425165\"]}");
	add("{\"term\":\"leucothoe\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"12260164\"]}");
	add("{\"term\":\"leucothoe editorum\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"12260397\"]}");
	add("{\"term\":\"leucothoe fontanesiana\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"12260397\"]}");
	add("{\"term\":\"leucothoe racemosa\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"12260667\"]}");
	add("{\"term\":\"leucotomy\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"00685381\"]}");
	add("{\"term\":\"leuctra\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"01286246\"]}");
	add("{\"term\":\"leukaemia\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"14266404\"]}");
	add("{\"term\":\"leukemia\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"14266404\"]}");
	add("{\"term\":\"leukeran\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"03025214\"]}");
	add("{\"term\":\"leukocyte\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"05457535\"]}");
	add("{\"term\":\"leukocytosis\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"14219703\"]}");
	add("{\"term\":\"leukoderma\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"14252730\"]}");
	add("{\"term\":\"leukoencephalitis\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"14366707\"]}");
	add("{\"term\":\"leukoma\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"14281065\"]}");
	add("{\"term\":\"leukopenia\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"14219887\"]}");
	add("{\"term\":\"leukorrhea\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"05425165\"]}");
	add("{\"term\":\"leukotomy\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"00685381\"]}");
	add("{\"term\":\"leuwenhoek\", \"synsetCount\":1, \"upperType\":\"NOUN\", \"ids\":[\"11149798\"]}");
 	}  	private static void add(final String JSON) { 		IndexNoun indexNoun = GsonUtils.toObject(JSON, IndexNoun.class); 		Collection<IndexNoun> list = (map.containsKey(indexNoun.getTerm())) ? map.get(indexNoun.getTerm()) : new ArrayList<IndexNoun>(); 		list.add(indexNoun); 		map.put(indexNoun.getTerm(), list); 	} 	 	public static Collection<IndexNoun> get(final String TERM) { 		return map.get(TERM); 	}  	public static boolean has(final String TERM) { 		return map.containsKey(TERM); 	} 	 	public static Collection<String> terms() { 		return map.keySet(); 	} }